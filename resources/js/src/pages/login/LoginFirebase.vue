<template>
  <div>
    <vs-input
        ...
        v-model="email"
        class="w-full"/>
    <span class="text-danger text-sm">{{ vErrors.first('email') }}</span>
    ...
    <vs-input
        ...
        v-model="password"
        class="w-full mt-6" />
    <span class="text-danger text-sm">{{ vErrors.first('password') }}</span>
...
export default {
  ...
  computed: {
    validateForm () {
      // return !this.vErrors.any() && this.email !== '' && this.password !== ''
    }
  },
  methods: {
    checkLogin () {
